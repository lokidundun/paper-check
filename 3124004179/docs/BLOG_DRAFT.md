GitHub 作业链接：[paper-check / 3124004179](https://github.com/lokidundun/paper-check/tree/main/3124004179)

# 第一次个人编程作业

学号：3124004179。课程作业：[个人项目——论文查重](https://edu.cnblogs.com/campus/gdgy/Class56-Grade2024-CS/homework/15693)。
...
当前实现为余弦计算直接遍历较小的频数字典，范数分别遍历已有频数，消除了特征并集和补零列表的额外开销。公式保持不变。

在固定种子 3124004179 生成的自建中文输入（原文 120,000 字符、待比较文本 126,000 字符）上：

| 指标                 |                优化后 |
| ------------------ | -----------------: |
| 核心计算中位耗时（5 次）      |           0.2342 秒 |
| Python 分配峰值        |          22.12 MiB |
| 含进程启动/文件读写的命令行中位耗时 |           0.4909 秒 |
| 得分                 | 0.8758433880625427 |

下列函数图由 SnakeViz 展示，对应优化版运行生成的 cProfile 报告。优化后的函数调用图与耗时排名见 `reports/images/` 目录。

图中的 `tottime` 是函数自身耗时，`cumtime` 包含子函数调用时间；排名按 `tottime` 降序展示。

## 四、单元测试与覆盖率

优化后执行 52 项测试全部通过（1 项符号链接测试因 Windows 权限自动跳过），覆盖正常输入、边界条件、异常分支和真实命令行调用。固定随机种子的额外检查验证相似度对称性、范围和优化前后公式等价性。
...
程序运行时代码共 93 条可执行语句、26 个分支，行与分支覆盖率均为 100%。覆盖范围不包括测试自身和开发工具，覆盖率也不能代替算法准确性评估。覆盖率截图见 `reports/images/` 目录。

Ruff 代码规则检查和格式检查通过。原始输出：[测试日志](https://github.com/lokidundun/paper-check/blob/main/3124004179/reports/tests.txt)、[覆盖率摘要](https://github.com/lokidundun/paper-check/blob/main/3124004179/reports/coverage-summary.txt)、[代码质量检查](https://github.com/lokidundun/paper-check/blob/main/3124004179/reports/quality.txt)。
...
以下三个示例均来自 [文件接口测试](https://github.com/lokidundun/paper-check/blob/main/3124004179/tests/test_fileio.py)。测试的 `setUp` 会在临时文件夹写入原文 `abc`、待比较文本 `abd`，预期正常结果为 `0.55`；`self.compare()` 调用三个临时文件路径对应的 `compare_files`。
...
本轮我主要补充了对算法步骤、边界情况和测试依据的理解，并把上述疑问及理解过程整理进博文。项目已有的测试和性能报告用于说明程序的验证结果：52 项测试通过，核心计算中位耗时 0.2342 秒，得分 0.8758。
