...
环境：Windows 11、x64、Python 3.12。核心优化后执行了 52 项 `unittest` 测试，全部通过（1 项符号链接测试因 Windows 权限自动跳过）。另有 150 组种子固定的随机文本性质检查，以及 100 组优化前后数学等价性检查，均包含在上述测试中。
...
![Coverage.py 实际生成的覆盖率报告](../reports/images/coverage-report.png)
