...
class FileTests(unittest.TestCase):
    ...
    def test_symlink_output_cannot_overwrite_input(self):
        # Windows 下创建符号链接需要管理员权限或开发者模式，无权限时跳过
        try:
            self.output.symlink_to(self.original)
        except OSError:
            self.skipTest("当前环境无创建符号链接的权限")
        with self.assertRaises(InvalidPathError):
            self.compare()
        self.assertEqual(self.original.read_text(), "abc")
    ...
    def test_write_permission_failure_is_explained(self):
        # patch 的模拟方法需要与 fileio 模块实际调用的写入方法一致
        # （当前实现使用 write_bytes，且 Windows 下 chmod 移除写权限不生效，故用 mock 验证）
